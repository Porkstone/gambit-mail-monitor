...
export const checkBookingEmailsForUser = internalAction({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; newMessages: number; error?: string }> => {
    ...
    if (!user)
      return { success: false, newMessages: 0, error: "User not found" };
    if (!user.gmailAccessToken)
    ...
    const refreshTokenIfNeeded = async (): Promise<string> => {
      ...
      await ctx.runMutation(internal.gmailHelpers.updateAccessToken, { userId: user._id, accessToken, expiresIn });
      user.gmailAccessToken = accessToken;
      user.gmailTokenExpiry = Date.now() + expiresIn * 1000;
      return accessToken;
    };
